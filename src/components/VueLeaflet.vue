<template>
  <div style="height: 100%; width: 100%">
    <l-map
      ...
      :min-zoom="minZoom"
      :max-zoom="maxZoom"
      style="height: 100%; width: 100%; background-color: #0c1321"
    >
      <l-tile-layer
      ...
      <l-layer-group layer-type="overlay" name="Routes">
        ...
        />
        <l-polyline
          v-for="route in route_obvious_list"
          :key="'highlight' + route.name"
          :lat-lngs="route.new_route_geo"
          :visible="true"
          :color="route_obvious_color"
          :weight="2"
          :opacity="0.9"
        />
        <l-marker
          v-for="vessel in vessel_run"
          :key="vessel.name"
          :visible="true"
          :draggable="false"
        ...
        <l-marker
          v-for="vessel in vessel_obvious_list"
          :key="'highlight' + vessel.name"
          :visible="true"
          :draggable="false"
          :lat-lng="vessel.position"
          :icon="icon_vessel_obvious"
        >
          <l-tooltip class="vessel_tooltip">
            <div>
              <span style="font-size: 12px; font-weight: bold">{{
              ...
              <div>
                ...
                <div style="width: 200px">
                  ...
                  <div style="float: left; width: 57%">
                    ...
                    <br />
                    <div style="font-size: 11px; color: grey">
                      {{ $t("port.booking") }}: {{ marker.acc_booking }} +
                      {{ marker.booking }}
                      <br />
                      {{ $t("port.laden") }}: {{ marker.acc_full }} +
                      {{ marker.full }}
                      <br />
                      {{ $t("port.empty") }}: {{ marker.acc_empty }} +
                      {{ marker.empty }}
                      <br />
                      {{ $t("port.fulfillment") }}:
                      {{ marker.acc_fulfillment }} + {{ marker.fulfillment }}
                      <br />
                      {{ $t("port.shortage") }}: {{ marker.acc_shortage }} +
                      {{ marker.shortage }}
                      <br />
                    </div>
                  </div>
    ...
    d3.json("/static/data/route.json").then((route_data) => {
      route_data.forEach((route) => {
        ...
        for (let i = 1; i < route.route_geo.length; i++) {
          for (let j = 1; j < 10; j++) {
            new_route_geo.push(
              this.get_accurate_vessel_position(
                route.route_geo[i - 1],
                route.route_geo[i],
                j / 10
              )
      ...
      d3.json(
        ...
        d3.json(
          `/static/data/epoch_${this.epoch}/vessel_run_data/vessel_run_${this.snapshot_number}.json`
        ).then((vessel_data) => {
    ...
    snapshot_number() {
      ...
      d3.json(
        ...
        d3.json(
          ...
          vessel_run_data = this.get_vessel_position(vessel_data);
          this.vessel_run = vessel_run_data
          // console.log(this.vessel_run, 'vessel_run');
          try {
            d3.json(
              `/static/data/epoch_${this.epoch}/decision_run_data/decision_run_${this.snapshot_number}.json`
            ).then(
              (cur_attention_port) => {
                let port_line = [];
                for (let item in cur_attention_port) {
                  for (let line_index in cur_attention_port[item][
                    "line_list"
                  ]) {
                    port_line.push({
                      port_idx: cur_attention_port[item]["port_idx"],
                      position: cur_attention_port[item]["line_list"][line_index],
                    });
                  }
                }
  ...
  methods: {
      ...
      for (let index = 0; index < this.markers.length; index++) {
        ...
        if (prev == [] || prev.length == 0 || this.play_count == 0) {
          ...
          if (prev[index].hasOwnProperty("acc_empty")) {
            this.markers[index].acc_empty =
              prev[index].acc_empty + prev[index].empty;
            this.markers[index].acc_shortage =
              prev[index].acc_shortage + prev[index].shortage;
            this.markers[index].acc_booking =
              prev[index].acc_booking + prev[index].booking;
            this.markers[index].acc_fulfillment =
              prev[index].acc_fulfillment + prev[index].fulfillment;
            this.markers[index].acc_full =
              prev[index].acc_full + prev[index].full;
          } else {
            this.markers[index].acc_empty = 0;
            this.markers[index].acc_shortage = 0;
            this.markers[index].acc_booking = 0;
    ...
    change_obvious_object() {
      ...
      for (let item in route_list) {
        ...
        for (
          let idx = start_vessel_idx;
          idx < start_vessel_idx + this.route_data[index].vessel_num;
          idx++
        ) {
    ...
    get_vessel_position(vessel_data) {
      ...
      for (let index = 0; index < vessel_data.length; index++) {
        ...
        let vessel_position_data = this.get_single_vessel_potision(
          route_vessel_info,
          route_info
        );
        all_vessel_data = all_vessel_data.concat(vessel_position_data);
      }
      // console.log(all_vessel_data, 'x');
    ...
    get_single_vessel_potision(route_vessel_info, route_info) {
      ...
      for (let index = 0; index < route_vessel_info.vessel.length; index++) {
        ...
        let current_distance =
          (vessel_speed * this.snapshot_number + start_position) %
          route_info.all_distance;
        let geo_position = {};
        let test_geo_position = {};
        if (current_distance > single_distance) {
          geo_position = this.get_appropriate_position(
            route_info,
            current_distance,
            port_num,
            0
    ...
    get_appropriate_position(route_info, current_distance, port_num, type) {
      ...
      if (type == 0) {
        // convert the "backward action" into "forward action"
        //physical distance between forward/backward are different, need to be converted
        current_distance =
          parseInt(route_info.single_distance) -
          (current_distance - route_info.single_distance) *
            (route_info.single_distance / route_info.route_config[0].distance);
      }
      ...
      segment_number =
        route_info.route_point.forward[parseInt(vessel_port_position)] + 1;
      // physical distance of this section of route
      segment_distance =
        route_info.route_config[parseInt(vessel_port_position) + 1].distance;
      ...
      surplus_all =
        current_distance -
        route_info.route_distance_list[parseInt(vessel_port_position)];
      ...
      for (
        let route_point_index = 0;
        route_point_index <= parseInt(vessel_port_position) - 1;
        route_point_index++
      ) {
        passed_other_stations += parseInt(
          route_info.route_point.forward[route_point_index]
        );
      }
      ...
      let station_just_pass =
        parseInt(vessel_port_position) +
        1 +
        parseInt(station_pass) +
        parseInt(passed_other_stations) -
        1;
      let res = {};
      if (surplus == 0) {
        res = route_info.route_geo[station_just_pass];
      } else {
        res = this.get_accurate_vessel_position(
          route_info.route_geo[station_just_pass],
          route_info.route_geo[station_just_pass + 1],
          surplus / segment_single
        );
      }
      return res;
    },
